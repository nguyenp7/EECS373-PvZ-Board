...
int main(void)
{
  ...
  HUB75_Init();
      ...
      render_view_init(&view, config.board_x_resolution, config.board_y_resolution, 0, 0);
  ...
  // HAL_TIM_Base_Start(&htim2);
	...
	while (1) {
		#ifdef MODE_TEST
        // Test mode — display is driven entirely by ISR, nothing to do in the main loop
        HAL_Delay(1000);
		#endif
			...
			{
				const float frame_dt = frame_dt_from_elapsed_us(elapsed_us, config.fixed_dt);
